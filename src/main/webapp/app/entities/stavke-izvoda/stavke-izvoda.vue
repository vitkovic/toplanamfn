    ...
    <div>
        ...
        <div class="table-responsive" v-if="stavkeIzvodas && stavkeIzvodas.length > 0">
            <table class="table table-striped">
                <thead>
                <tr>
                    <th v-on:click="changeOrder('datum')"><span v-text="$t('toplanaApp.stavkeIzvoda.datum')">Datum</span> <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'datum'"></jhi-sort-indicator></th>
                    ...
                    <th v-on:click="changeOrder('status')"><span v-text="$t('toplanaApp.stavkeIzvoda.status')">Status</span> <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'status'"></jhi-sort-indicator></th>
                    ...
                    <th v-on:click="changeOrder('sifra')"><span v-text="$t('toplanaApp.stavkeIzvoda.sifra')">Sifra</span> <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'sifra'"></jhi-sort-indicator></th>
                    <th v-on:click="changeOrder('opis')"><span v-text="$t('toplanaApp.stavkeIzvoda.opis')">Opis</span> <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'opis'"></jhi-sort-indicator></th>
                ...
                <tr v-for="stavkeIzvoda in stavkeIzvodas"
                    :key="stavkeIzvoda.id">
                    <td>{{stavkeIzvoda.datumValute}}</td>
                    <td>{{stavkeIzvoda.rasporedjena}}</td>
                    
                    <td>{{stavkeIzvoda.status}}</td>
                    <td>{{stavkeIzvoda.iznos}}</td>
                    <td>{{stavkeIzvoda.sifra}}</td>
                    <td>{{stavkeIzvoda.opis}}</td>
                    <td>
                    ...
                    <!--
                    <td>
                        <div v-if="stavkeIzvoda.transakcijaStara">
                    ...
                    <td>
                        <div v-if="stavkeIzvoda.izvod">
                            <router-link :to="{name: 'IzvodView', params: {izvodId: stavkeIzvoda.izvod.id}}">{{stavkeIzvoda.izvod.brojIzvoda}}</router-link>
                        </div>
                    </td>
                    <td class="text-right">
                        <div class="btn-group">
...
<script lang="ts" src="./stavke-izvoda.component.ts">
</script>
